...
mod tests {
    ...
    fn deliver(
        ...
        resolve_owner(
            ...
            |window| {
                windows
                    .get(&window)
                    .is_some_and(|children| children.contains(&browser))
            },
            || {
                windows
                    .iter()
                    .find_map(|(window, children)| children.contains(&browser).then_some(*window))
            },
    ...
    fn unchanged_owner_does_not_scan_other_windows() {
        assert_eq!(
            resolve_owner(1, true, |_| true, || panic!("unnecessary scan"), |_| false),
            Some(1)
        );
    }
}
