...
use std::{
  ...
  sync::{
    Arc, Mutex,
    atomic::{AtomicBool, AtomicI32, AtomicPtr, AtomicU32, Ordering},
    ...
    match message {
      ...
      Message::BrowserClosed(_window_id, webview_id) => {
        ...
        if let Some(window_id) = emptied_window {
          if self.state.exiting || self.state.closing_windows.contains(&window_id) {
      ...
      Message::DestroyWebviewHostWindow(webview_id) => {
        ...
        //
        // Closing windows remain registered until BrowserClosed acknowledges
        // every child, including during whole-application shutdown.
      ...
      Message::CreateWindow {
        ...
        if self.state.exiting {
          let _ = result_tx.send(Err(Error::CreateWindow));
          return;
        }
      ...
      Message::CreateWebview {
        ...
        if self.state.exiting || self.state.closing_windows.contains(&window_id) {
          let _ = result_tx.send(Err(Error::WindowNotFound));
          return;
        }
      ...
      Message::Webview {
        ...
        // Retaining closing windows must not make them valid reparent targets
        // or let a closing child escape to a different parent mid-teardown.
        if let WebviewMessage::Reparent(target, ref tx) = message
          && (self.state.exiting
            || self.state.closing_windows.contains(&window_id)
            || self.state.closing_windows.contains(&target))
        {
  ...
  fn request_exit(&mut self, code: Option<i32>) -> bool {
    ...
    if matches!(rx.try_recv(), Ok(ExitRequestedEventAction::Prevent)) {
      ...
      self
        .state
        .exit_code
        .store(code.unwrap_or(0), Ordering::SeqCst);
      true
    }
  }
