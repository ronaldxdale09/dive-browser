...
impl PermissionBridge {
    ...
    fn complete(&self, id: u64, allowed: Option<bool>) {
        ...
        if let Some(pending) = pending {
            let allowed =
                allowed.filter(|_| Instant::now() < pending.deadline && (pending.valid)());
        ...
        let (ids, navigation) = {
            ...
            (
                state
                    ...
                    .iter()
                    .filter(|(_, request)| {
                        main || request.frame_id.is_none() || request.frame_id == frame_id
                    })
                    .map(|(id, _)| *id)
                    .collect::<Vec<_>>(),
                ...
                Pending {
                    ...
                    valid: Box::new(move || {
                        check_browser.is_valid() != 0
                            && check_browser.main_frame().is_some_and(|frame| {
                                CefString::from(&frame.url()).to_string() == check_top
                            })
