...
### Added
- An Apps launcher (⌘⇧Space, or the Apps button in the bar): every feature Dive has on one screen, in shelves (Capture, On the page, Developer, Privacy, Library, Dive), each with a line on what it is for and the shortcut that reaches it. Type to search across the shelves, arrow keys walk the grid, Enter opens the first match; cards that need a page grey out when no tab is open. The bar beside the address keeps only the page's own actions (bookmark, share) and what is happening to it (zoom, subtitles, a recording of steps, a download in progress), and the feature cluster keeps only Agent and Apps; the developer dock, DevTools, extensions, the device simulator, capture and downloads are one click further, in Apps. The build badge moved into the launcher's header.
