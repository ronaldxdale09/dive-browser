...
### Added
- An Apps launcher (⌘⇧Space, or the Apps button in the bar): Dive's twelve tools on one screen, shelved as Capture (DiveScreen, Screenshot, Test recorder), On the page (Agent, Live subtitles), Developer (Developer dock, DevTools, Device simulator, Extensions) and Yours (DivePrivacy, Passwords & forms, Library), each with a line on what it is for and the shortcut that reaches it. Commands such as find, print, settings and new window are not apps and stay in the menu and the palette. Type to search across the shelves, arrow keys walk the grid, Enter opens the first match; cards that need a page grey out when no tab is open. The bar beside the address keeps only the page's own actions (bookmark, share) and what is happening to it (zoom, subtitles, a recording of steps, a download in progress), and the feature cluster keeps only Agent and Apps; the developer dock, DevTools, extensions, the device simulator, capture and downloads are one click further, in Apps. The build badge moved into the launcher's header.
...
### Changed
- The rail earns its width: under the workspaces it lists the current workspace's tabs, and while it does the title bar shows none, so a tab is in one place. The rail list is the same strip turned on its side: reorder by dragging, drag onto the page to split, right-click for the tab menu, arrow keys to move, essentials and pinned first, the current one marked, close from the row, plus for a new tab. Collapse the rail and the tabs return to the title bar. And with the tabs in the rail, the title bar folds into the toolbar: one row of navigation, address, page actions, the feature cluster (Capture, Mobile, Agent) and, in the corner, privacy and the menu; the rail runs the full height with the traffic lights at its top. The profile switcher moved from the title bar to the foot of the rail, under the workspaces it owns, so profile and workspace read as one column. The "AI shortcuts" row became "Quick links": the same three assistants to start with, but yours to add to (plus, any web address) and remove (hover a link), drawn in grey until hovered so the rail stays quiet. The "own cookies" mark on a workspace shows for the active row and under the pointer rather than on every row, the "Set as default" card has a "Not now" that puts it away for two weeks, and the rail's section labels are a size larger.
- Record and the toolbar's camera became one "Capture" control in the title bar, with a menu for a video or GIF recording and a full-page screenshot; the recording's live controls still take its place while one runs. The toolbar's icons now sit in three groups: the page (bookmark, share), the developer surfaces (dock, DevTools with a code glyph instead of a bug, extensions), then downloads, privacy and the menu.
- The command palette leads with open tabs, then local servers, history, bookmarks and commands, in that order, whether or not something has been typed; its group headings match the chrome's labels, a footer says what the keys do (move, open, close), and assistive tech hears "Command palette" rather than "New tab".
- The agent panel speaks plainly: the empty conversation says what to do ("Ask about the page you are on, or tell the agent what to do in it") instead of a second title and a line of jargon, the starters are "Summarize this page", "Find what is broken", "Check it on a phone" and "Check accessibility", the setup screen explains the choice in one sentence, the page badge says "off" instead of striking itself through, and the panel's type sits on the chrome's 11 and 12 px scale.
- Every dialog now rises over the same scrim (the ground's own tone, softly blurred) instead of some using it and others a flat black; the Apps launcher sizes itself to its content and the window (three cards across on a desktop, two below 900px, one below 640px), shows a proper focus ring on the card the keyboard is on, and no longer carries the build badge in its header (About has it).
- The address bar reads the site first: at rest the host is set in ink and the path in a quieter tone, and the glyph in front says what the connection is (a lock for https, a globe for plain http, a warning for a page that did not load, a magnifier only when the bar is empty), each explaining itself on hover.
- The main menu no longer lists the tools a second time: one "Apps" row points at the launcher, and the menu is window, tab, library, zoom, page and Dive, short enough to read without its search box. "Edit latest recording" and "Open a video in DiveScreen" sit with Recordings, "Copy bug report" with the page actions, and About wears an info glyph rather than a bug.
- The rail's collapse control lives in one place, the top of the rail, in both states, and says what it does ("Collapse the rail", "Expand the rail"); it used to sit in the Workspaces header when expanded and at the bottom when collapsed. Collapsed, every mark explains itself on hover with the chrome's own tooltip (name, tab count, own-cookies note and the ⌘1–9 chord), as do the plus, Settings and the default-browser offer, instead of a native title box.
- The development-build pill is amber rather than red, so it stops reading as "recording" next to Capture, and explains itself on hover.
