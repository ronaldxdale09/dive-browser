...
const started = { url: "https://cdn.example.com/report.pdf", path: "/dl/report.pdf", status: "started" };
...
describe("downloads fold", () => {
  ...
  it("matches a failure by URL when the engine has no path for it", () => {
    const one = fold([], started, 1000);
    const failed = fold(one, { url: started.url, path: "", status: "failed" }, 3000);
  ...
  it("keeps the newest first and caps the list", () => {
    ...
    for (let i = 0; i < 60; i++) items = fold(items, { url: `https://x/${i}`, path: `/dl/${i}`, status: "finished" }, i + 2);
