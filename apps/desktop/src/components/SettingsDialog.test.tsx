...
describe("SettingsDialog", () => {
  ...
  it("closes on Escape", async () => {
    useBrowser.setState({ open: { sidecar: false, dock: false, palette: false, find: false, settings: true, library: false, shortcuts: false, menu: false, defaultBrowser: false, subtitles: false, tasks: false } });
    render(<SettingsDialog />);
...
describe("private window", () => {
  it("lists only the sections private mode can use, and no import", async () => {
    ...
    expect(visibleSections(false)).toHaveLength(10);
